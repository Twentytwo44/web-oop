     ...
     <div className='flex flex-col mt-9 border border-gray-100 rounded' >
       {
            ...
            <thead className='capitalize text-base text-gray-100 font-medium borde-b border-gray-100'>
              <tr>
                  <th className='py-1'>asset</th>
                  <th className='py-1'>name</th>
                  <th className='py-1'>price</th>
                  <th className='py-1'>total volumn</th>
                  <th className='py-1'>market cap change</th>
                  <th className='py-1'>1H</th>
                  <th className='py-1'>24D</th>
                    ...
                    cryptoData.map(data => {
                       return(
                        ...
                        hover:bg-gray-200 last:border-b-0'>
                            <td className='py-4 text-white flex items-center uppercase '>
                                <button className='outline-o border-0 cursor-pointer'>
                                <svg 
                                className='w-[1.5rem] ml-1.5 fill-gray-100 hover:fill-cyan-500'
                                width="30" 
                                height="30" 
                                viewBox="0 0 30 30" 
                                fill="cyan" 
                            ...
                            <td className='py-4 text-white'>{data.name}</td>
                            <td className='py-4 text-white'>{
                                new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'usd' }).format(data.current_price)
                            }</td>
                            <td className='py-4 text-white'>{data.total_volume}</td>
                            <td className='py-4 text-white'>{data.market_cap_change_percentage_24h}%</td>
                            <td className={data.price_change_percentage_1h_in_currency >  0 ? 'text-[#008000] py-4' : 'text-[#FF0000] py-4'}>{Number(data.price_change_percentage_1h_in_currency).toFixed(2)}</td>
                            <td className={data.price_change_percentage_24h_in_currency >  0 ? 'text-[#008000] py-4' : 'text-[#FF0000] py-4'}>{Number(data.price_change_percentage_24h_in_currency).toFixed(2)}</td>
                            <td className={data.price_change_percentage_7d_in_currency >  0 ? 'text-[#008000] py-4' : 'text-[#FF0000] py-4'}>{Number(data.price_change_percentage_7d_in_currency).toFixed(2)}</td>
                        </tr>
                       )
                    })
                }
